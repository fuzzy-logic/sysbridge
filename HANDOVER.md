...
| 4 systemd | **installed 2026-09-22 via `./install.sh`**: `sysbridge-http` enabled, port 80, all 11 probes ok under the hardening | every probe passes under `systemd-run -p ProtectSystem=strict -p ProtectHome=read-only -p NoNewPrivileges=true -p PrivateTmp=true` |
...
1. ~~Run `./install.sh`~~ — done by the user 2026-09-22; `http://localhost/` is live.
   Reverse with `./install.sh --uninstall [--purge-sysctl]`.
...
**Left for the user:** (1) ~~`./install.sh`~~ done: `sysbridge-http` runs on port 80,
verified from the service (health, all probes, launcher, dashboard). (2) The
